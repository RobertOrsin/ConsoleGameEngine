...
namespace ProjectDMG {
    public class PPU {
        ...
        private char[] pixel = new char[] { (char)0x2588, (char)0x2593, (char)0x2592, (char)0x2591 };
       ...
       // public DirectBitmap bmp;
        ...
        public PPU(GameConsole gameConsole) {
        ...
        private void renderBG(MMU mmu) {
            ...
            for (int p = 0; p < SCREEN_WIDTH; p++) {
              ...
              //  bmp.SetPixel(p, LY, color[colorIdThroughtPalette]);
                gameConsole.SetChar(p, LY, pixel[colorIdThroughtPalette]);
            }
                    ...
                    for (int p = 0; p < 8; p++) {
                        ...
                        if ((x + p) >= 0 && (x + p) < SCREEN_WIDTH) {
                            if (!isTransparent(colorId) && (isAboveBG(attr) || isBGWhite(mmu.BGP, x + p, LY))) {
                                gameConsole.SetChar(x + p, LY, pixel[colorIdThroughtPalette]);
                            }
        ...
        private bool isBGWhite(byte BGP, int x, int y) {
            int id = BGP & 0x3;
            return gameConsole.GetChar(x, y) == pixel[id];
        }
